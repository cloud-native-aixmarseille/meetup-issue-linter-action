...
export const MEETUP_ISSUE_BODY_FIELD_LABELS: Record<
...
export class MeetupIssueService {
	...
	async getMeetupIssue(
		...
		const { number, title, labels, body } =
			await this.githubService.getIssue(issueNumber);
	...
	async updateMeetupIssue(
		originalIssue: MeetupIssue,
		updatedIssue: MeetupIssue,
	): Promise<void> {
	...
	updateMeetupIssueBodyField(
		meetupIssue: MeetupIssue,
		field: MeetupIssueBodyFields,
	): void {
