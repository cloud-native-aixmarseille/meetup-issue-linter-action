...
describe("AgendaLinterAdapter", () => {
	...
	describe("lint", () => {
		...
		])("should return the meetup issue if the Agenda is valid with $description", async ({
			agenda,
		}) => {
			// Arrange
		...
		it("should fix the Agenda if shouldFix is true", async () => {
			...
			expect(
				meetupIssueService.updateMeetupIssueBodyField,
			).toHaveBeenCalledWith(meetupIssue, "agenda");
		...
		it.each([
			...
			await expect(
				agendaLinterAdapter.lint(invalidMeetupIssue, shouldFix),
