	...
	describe("lint", () => {
		...
		it("should expect hoster needed label when hoster is empty", async () => {
			...
			const expectedError = new LintError([
				`Labels: Missing label(s) "hoster:needed"`,
			]);
		...
		it("should expect speaker needed label when agenda empty", async () => {
			...
			// Assert
			const expectedError = new LintError([
				'Labels: Missing label(s) "speakers:needed"',
			]);
			await expect(result).rejects.toStrictEqual(expectedError);
		});
		...
		it.each([
			{
				...
				labels: ["Wrong Label"],
				errors: [
					'Missing label(s) "meetup, hoster:confirmed"',
					'Extra label(s) "Wrong Label"',
				],
			},
			{
			...
			// Act & Assert
			const expectedError = new LintError(
				errors.map((error) => `Labels: ${error}`),
			);

			await expect(
				labelsLinterAdapter.lint(invalidMeetupIssue, shouldFix),
		...
		it("should fix the labels if it is invalid and shouldFix is true", async () => {
			...
			// Act
			const result = await labelsLinterAdapter.lint(
				invalidMeetupIssue,
				shouldFix,
			);
