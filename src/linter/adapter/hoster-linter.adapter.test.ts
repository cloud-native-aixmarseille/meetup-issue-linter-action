...
describe("HosterLinterAdapter", () => {
	...
	beforeEach(() => {
		...
		hosterLinterAdapter = new HosterLinterAdapter(
			meetupIssueService,
			inputServiceMock,
		);
	});
	...
	describe("lint", () => {
		...
		it.each([
			...
			await expect(
				hosterLinterAdapter.lint(invalidMeetupIssue, shouldFix),
		...
		it("should add links to hoster when shouldFix is true", async () => {
			...
			expect(
				meetupIssueService.updateMeetupIssueBodyField,
			).toHaveBeenCalledWith(meetupIssue, "hoster");
			expect(result.parsedBody.hoster).toEqual([
		...
		it("should be idempotent for hoster having link", async () => {
			...
			// Act
			const resultWithLink = await hosterLinterAdapter.lint(
				meetupIssueWithLink,
				shouldFix,
			);
