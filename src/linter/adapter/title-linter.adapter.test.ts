...
describe("TitleLinterAdapter", () => {
	...
	describe("lint", () => {
		...
		it.each([
			{
				...
				title: "Wrong Title",
				error:
					'Invalid, expected "[Meetup] - 2021-12-31 - December - Meetup Event"',
			},
			...
			await expect(
				titleLinterAdapter.lint(invalidMeetupIssue, shouldFix),
		...
		it("should fix the title if it is invalid and shouldFix is true", async () => {
			...
			// Act
			const result = await titleLinterAdapter.lint(
				invalidMeetupIssue,
				shouldFix,
			);
			...
			expect(result.title).toBe(
				"[Meetup] - 2021-12-31 - December - Meetup Event",
			);
		});
		...
		])("should throw an error if $description", async ({
			parsedBody,
			error,
		}) => {
			// Arrange
