...
export class LinterService {
	constructor(
		@multiInject(LINTER_ADAPTER_IDENTIFIER)
	...
	async lint(meetupIssue: MeetupIssue, shouldFix: boolean): Promise<void> {
		...
		let linter = linterQueue.dequeue();
		while (linter) {
			const lintResult = await this.runSingleLinter(
				linter,
				lintedMeetupIssue,
				shouldFix,
			);
			...
			linter = linterQueue.dequeue();
		}

		if (shouldFix) {
			await this.meetupIssueService.updateMeetupIssue(
				meetupIssue,
				lintedMeetupIssue,
			);
		}
